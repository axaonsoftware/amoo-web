  ...
  return (
    <section className="rounded-[16px] border border-[#f0eaf8] bg-white p-5 shadow-[0_1px_3px_rgba(42,17,72,.05)]">
      <div className="flex items-center justify-between gap-4">
        <h2 className="font-display text-[17px] font-bold text-[#3d1a63]">Latest Bookings</h2>
        <Link href="/admin/booking-management" className="text-[11.5px] font-medium text-[#7c3aed]">
          View All
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <ul className="mt-4 space-y-[14px]">
          {rows.slice(0, 5).map((a: { booking_ref: string; user_name: string; service_name: string; amount: number; status: string; payment: string }) => (
            <li key={a.booking_ref} className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[12px] font-semibold text-[#2a1148]">{a.service_name}</p>
                <p className="mt-[2px] truncate text-[10.5px] text-[#8b8397]">{a.user_name}</p>
              </div>
