...
export default function RefundsHistory() {
  ...
  useEffect(() => {
    setLoading(true);
    api.admin
      .getPaymentRefunds(`?page=${page}&pageSize=${itemsPerPage}`)
      .then((res) => {
  ...
  return (
    <section className="rounded-[14px] border border-[#EFEDF4] bg-white p-[18px] shadow-[0_1px_2px_rgba(16,12,40,0.03)]">
      ...
      {loading ? (
        ...
        <p className="mt-4 text-[12px] text-[#8B879C]">
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="border-b border-[#F5F3F9]">
                    <td className="py-[10px] pr-3 text-[11px] font-medium text-[#4A3B63]">
                      #{r.id}
                    </td>
                    <td className="py-[10px] pr-3 text-[11px] text-[#4A3B63]">
                      {r.txn_id || `#${r.payment_id}`}
                    </td>
                    <td className="py-[10px] pr-3 text-[11px] font-medium text-[#1B1630]">
                      {fmtAmount(r.amount)}
                    </td>
                    <td className="max-w-[180px] truncate py-[10px] pr-3 text-[11px] text-[#4A3B63]">
                      {r.reason || "\u2014"}
                    </td>
                    <td className="py-[10px] pr-3">
                      <span
                        className={`inline-flex h-[20px] items-center rounded-full px-[9px] text-[9.5px] font-medium capitalize ${
                          r.status === "processed"
                            ? "bg-[#E3F7EA] text-[#16A34A]"
                            : r.status === "failed"
