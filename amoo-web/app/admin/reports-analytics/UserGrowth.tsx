...
const fallbackUsers = [760, 720, 880, 1020, 1180, 1120, 1560, 1480, 1720, 1780, 1700, 1860, 2020, 1940, 2320, 2180, 2360, 2440];
...
export default function UserGrowth() {
  ...
  useEffect(() => {
    ...
    api.admin.getUsersGrowth("month")
      ...
      .catch((err) => { if (!cancelled) setError(err?.message || "Failed to load user growth"); })
  ...
  return (
    <section className="rounded-[14px] border border-[#EFEDF4] bg-white p-[18px] shadow-[0_1px_2px_rgba(16,12,40,0.03)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <h2 className="text-[14px] font-semibold text-[#1B1630]">User Growth</h2>
        <div className="flex items-start gap-4">
          ...
          <div className="text-right">
            ...
            <p className="text-[17px] font-semibold leading-[22px] text-[#1B1630]">{total}</p>
      ...
      {error ? (
        ...
        <div className="mt-4 flex gap-2">
          <div className="flex w-[24px] shrink-0 flex-col justify-between py-[2px] text-right text-[9px] text-[#A5A2B5]">
            {["3.0K", "2.4K", "1.8K", "1.2K", "600", "0"].map((t) => <span key={t}>{t}</span>)}
          </div>
          <div className="min-w-0 flex-1">
            <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="h-[160px] w-full overflow-visible">
              <defs>
              ...
              {users.map((v, i) => <circle key={i} cx={x(i)} cy={y(v)} r="3" fill="#6D28D9" vectorEffect="non-scaling-stroke" />)}
            </svg>
