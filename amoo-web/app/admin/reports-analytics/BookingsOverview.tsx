...
const fallbackBookings = [96, 132, 108, 152, 118, 176, 122, 138, 168, 128, 158, 172, 148, 166];
const fallbackCompleted = [72, 60, 84, 66, 92, 74, 96, 68, 84, 74, 90, 76, 106, 80];
        ...
        <>
          <div className="mt-3 flex items-center justify-center gap-5">
            <span className="flex items-center gap-[6px] text-[10px] font-medium text-[#6C6B78]">
              <span className="h-[8px] w-[8px] rounded-[2px] bg-[#6D28D9]" /> Bookings
            </span>
            <span className="flex items-center gap-[6px] text-[10px] font-medium text-[#6C6B78]">
              <span className="h-[8px] w-[8px] rounded-[2px] bg-[#F59E0B]" /> Completed
            </span>
          </div>
            ...
            <div className="min-w-0 flex-1">
              <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="h-[168px] w-full overflow-visible">
                ...
                {currentBookings.map((v, i) => (
                  <rect key={i} x={cx(i) - BAR / 2} y={y(v)} width={BAR} height={H - y(v)} rx="3" fill="#6D28D9" />
                ))}
                <polyline points={linePoints} fill="none" stroke="#F59E0B" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke" />
                {completed.map((v, i) => (
                  <circle key={i} cx={cx(i)} cy={y(v)} r="2.5" fill="#F59E0B" vectorEffect="non-scaling-stroke" />
                ))}
              </svg>
              <div className="mt-[10px] flex justify-between text-[9px] text-[#A5A2B5]">
                {["1 May", "5 May", "9 May", "13 May", "18 May"].map((l) => <span key={l}>{l}</span>)}
              </div>
            </div>
          </div>
        </>
