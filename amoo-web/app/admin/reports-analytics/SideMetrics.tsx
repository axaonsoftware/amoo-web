...
export default function SideMetrics() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([api.admin.getPayments(), api.admin.getOverview()])
      .then(([paymentsRes, overviewRes]) => {
        if (cancelled) return;
        const payments: Payment[] = Array.isArray(paymentsRes?.data)
          ? paymentsRes.data
        ...
        if (payments.length > 0) {
          ...
          for (const p of payments) {
            if (p.created_at) {
              ...
              dailyBuckets.set(
                day,
                (dailyBuckets.get(day) || 0) + (p.amount || 0),
              );
            }
          }
          ...
          if (sparkline.length >= 5) {
            ...
            setSpark(
              sparkline.map((v) => ((v - min) / (max - min || 1)) * 26 + 2),
            );
            setSparkPayouts(
              sparkline.map(
                (v) =>
                  ((v * 0.63 - min * 0.63) / (max * 0.63 - min * 0.63 || 1)) *
                    22 +
                  2,
              ),
            );
  ...
  return (
    <section className="flex flex-col rounded-[14px] border border-[#EFEDF4] bg-white p-[18px] shadow-[0_1px_2px_rgba(16,12,40,0.03)]">
      {error ? (
        ...
        <>
          <div>
            <h2 className="text-[13px] font-semibold text-[#1B1630]">
              Conversion Rate
            </h2>
            <div className="mt-3 flex items-center gap-3">
              <svg viewBox="0 0 76 76" className="h-[68px] w-[68px] shrink-0">
                ...
                <text
                  ...
                  y="41"
                  textAnchor="middle"
                  className="fill-[#1B1630] text-[11px] font-semibold"
                >
                  {pct}%
                </text>
              </svg>
              <div className="min-w-0">
                <p className="flex flex-wrap items-center gap-[3px] text-[9.5px] text-[#A5A2B5]">
                  vs last month
                  <span className="flex items-center gap-[2px] font-medium text-[#16A34A]">
                    <ArrowUp size={9} />
                    8.7%
                  </span>
                </p>
                <p className="mt-[3px] text-[9.5px] text-[#A5A2B5]">
                  {paidCount} paid out of 100
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 border-t border-[#F1EFF6] pt-4">
            <h3 className="text-[12px] font-semibold text-[#1B1630]">
              Average Order Value
            </h3>
            <div className="mt-2 flex items-end justify-between gap-2">
              <div className="min-w-0">
                <p className="whitespace-nowrap text-[17px] font-semibold leading-[23px] text-[#1B1630]">
              ...
              <svg
                ...
                className="h-[24px] w-[68px] shrink-0 overflow-visible"
              >
                <polyline
          ...
          <div className="mt-4 border-t border-[#F1EFF6] pt-4">
            ...
            <div className="mt-2 flex items-end justify-between gap-2">
              ...
              >
                <polyline
                  points={sparkPoints(sparkPayoutsArr)}
                  fill="none"
                  stroke="#A855F7"
                  strokeWidth="1.6"
                  strokeLinejoin="round"
