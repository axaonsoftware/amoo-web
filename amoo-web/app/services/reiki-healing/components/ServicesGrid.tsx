            ...
            const match = ICON_TONE_MAP[svc.name] ||
              ICON_TONE_MAP[
                Object.keys(ICON_TONE_MAP).find((k) =>
                  svc.name.includes(k.replace(/ .*/, "")),
                ) || ""
              ] || {
                icon: PersonalHealingIcon,
                tone: svc.name.length % 2 === 0 ? "purple" : ("gold" as const),
              };
            const Icon = match.icon;
            const tone = match.tone;
            const titleA = svc.name.includes(" ")
              ? svc.name.substring(0, svc.name.indexOf(" "))
              : svc.name;
            const titleB = svc.name.includes(" ")
              ? svc.name.substring(svc.name.indexOf(" ") + 1)
              : "";
