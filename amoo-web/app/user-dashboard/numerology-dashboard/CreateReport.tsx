  ...
  const handleCreate = async () => {
    ...
    try {
      await api.createReport({
        type: "numerology",
        report_type: selectedType.type,
        title: selectedType.title,
      });

      setMessage(`${selectedType.title} created successfully!`);
    } catch (e: unknown) {
  ...
  return (
    <section className="rounded-[14px] border border-[#ece9f3] bg-white px-5 py-[16px] shadow-[0_1px_3px_rgba(43,15,71,.04)]">
      ...
      <div className="mt-3.5 grid grid-cols-2 gap-[10px] sm:grid-cols-3">
        {reportTypes.map(({ title, sub, type, Icon, iconBg, iconColor }) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center rounded-[12px] border px-2 py-[10px] text-center transition ${
                selected
                  ? "border-[#6d28d9] bg-[#faf7ff] shadow-[0_0_0_1px_#6d28d9]"
                  : "border-[#efecf6] bg-white shadow-[0_1px_2px_rgba(43,15,71,.03)]"
              }`}
            >
              <span
