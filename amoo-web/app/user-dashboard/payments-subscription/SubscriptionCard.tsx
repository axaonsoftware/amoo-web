...
type Sub = { id: number; name: string; created_at: string; amount: number | string; status: string };
...
export default function SubscriptionCard() {
  ...
  const planDetails = sub ? [
    ...
    { label: "Start Date", value: sub.created_at ? new Date(sub.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) : "—" },
    { label: "Status", value: sub.status || "Active" },
    { label: "Amount", value: `₹${Number(sub.amount || 0).toLocaleString("en-IN")}` },
  ] : [
  ...
  return (
    <section className="rounded-[16px] border border-[#f1e8da] bg-white p-[18px] shadow-[0_1px_3px_rgba(43,15,71,0.04)]">
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-display text-[17px] font-bold text-[#2b0f47]">My Subscription</h3>
        <Link
      ...
      <div className="mt-[14px] rounded-[14px] border border-[#f0e2c8] bg-[#fdf8ef] p-[15px]">
        <div className="flex items-start gap-3">
          <span className="flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full bg-[#fbeed2]">
            <Crown className="h-[21px] w-[21px] text-[#e9b85c]" strokeWidth={1.7} />
          </span>
          <div className="min-w-0 flex-1">
            <p className="font-display text-[15px] font-bold text-[#2b0f47]">{sub?.name || "Free Plan"}</p>
            <p className="mt-[4px] text-[11.5px] leading-[1.5] text-[#6c6b78]">
              {sub ? "Manage your subscription benefits." : "No active subscription."}
            </p>
          </div>
        ...
        <div className="mt-[16px] flex flex-col gap-[10px]">
          {planDetails.map((row) => (
            <div key={row.label} className="flex items-center justify-between">
              ...
              <span className="text-[12px] font-semibold text-[#2b0f47]">{row.value}</span>
            </div>
          ))}
        </div>
