...
export default function AvailableOffers() {
  const { data: packages, loading, error } = useApi<any[]>(() => api.getPackages());
  const offers = (packages ?? []).filter((p: any) => p.status !== "inactive").slice(0, 2).map((p: any) => ({
  ...
  return (
    <section className="rounded-[16px] border border-[#f1e8da] bg-white p-[18px] shadow-[0_1px_3px_rgba(43,15,71,0.04)]">
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-display text-[17px] font-bold text-[#2b0f47]">Available Offers</h3>
        <Link
      ...
      {loading ? (
        ...
        <p className="mt-[14px] text-[12.5px] text-[#8b8697]">No offers available.</p>
      ) : (
      <div className="mt-[14px] flex flex-col gap-[12px]">
        {offers.map(({ code, desc, Icon, tint, iconColor }) => (
          ...
          >
            ...
            >
              <Icon className={`h-[17px] w-[17px] ${iconColor}`} strokeWidth={2} />
            </span>
            <div className="min-w-0 flex-1">
              <p className="text-[13px] font-semibold text-[#2b0f47]">{code}</p>
