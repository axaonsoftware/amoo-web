...
export default function RecentReports() {
  const { data, loading, error } = useApi<{ data: Report[] }>(() => api.getReports(), [], 15000);
  ...
  return (
    <section className="rounded-[16px] border border-[#efe6d6] bg-white px-5 pb-5 pt-4 shadow-[0_2px_10px_rgba(42,17,72,.05)]">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-[17px] font-bold text-[#4a1c7d]">Recent Reports</h2>
        <Link href="/user-dashboard/my-reports" className="text-[12px] font-medium text-[#6b3fa0]">View All</Link>
      </div>

      {loading ? (
        <p className="mt-4 text-[12.5px] text-[#8b8697]">Loading reports...</p>
      ) : error ? (
        <p className="mt-4 text-[12.5px] text-red-600">Failed to load reports.</p>
      ) : rows.length === 0 ? (
        ...
        <div className="mt-3 flex flex-col">
          {rows.map((r, i) => {
            ...
            return (
              <div key={r.id} className={`flex items-center gap-3 py-3.5 ${i === 0 ? "" : "border-t border-[#f1ebe0]"}`}>
                ...
                <Link href="/user-dashboard/my-reports" className="shrink-0 rounded-[8px] border border-[#e0d3ee] bg-[#f7f2fd] px-5 py-2 text-[12px] font-semibold text-[#4a1c7d]">
                  View
                </Link>
              </div>
            );
