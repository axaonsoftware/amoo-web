...
type Booking = { id: number; service_name: string; date: string; status: string };

export default function RecentSessions() {
  const { data, loading, error } = useApi<{ data: Booking[] }>(() => api.getBookings());
  const sessions: Booking[] = (data?.data ?? []).filter(
  ...
  return (
    <section className="rounded-[14px] border border-[#ece9f3] bg-white px-5 py-[18px] shadow-[0_1px_3px_rgba(43,15,71,.04)]">
      ...
      {loading ? (
        ...
        <p className="mt-3.5 text-[12.5px] text-[#8b8697]">No completed sessions yet.</p>
      ) : (
        ...
        {sessions.map((s) => {
          ...
          <div key={s.id} className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[12.5px] font-semibold text-[#2b0f47]">{s.service_name}</p>
