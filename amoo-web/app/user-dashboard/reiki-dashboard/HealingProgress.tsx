...
function deriveChartData(reports: Report[]) {
  if (reports.length === 0) {
    return { weeks: ["Week 1", "Week 2", "Week 3", "Week 4", "This Week"], values: [0, 0, 0, 0, 0] };
  }
  const now = Date.now();
  ...
  const values = buckets.map((count) => Math.round((count / Math.max(total, 1)) * 100));
...
export default function HealingProgress() {
  ...
  const { weeks, values } = useMemo(() => deriveChartData(reikiReports), [reikiReports]);
  ...
  return (
    <section className="rounded-[14px] border border-[#ece9f3] bg-white px-5 py-[18px] shadow-[0_1px_3px_rgba(43,15,71,.04)]">
      ...
      {loading ? (
        ...
        <p className="mt-4 text-[12.5px] text-[#8b8697]">No healing sessions yet. Start your first session to track progress.</p>
      ) : (
      <div className="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-[auto_1fr_1.15fr] lg:items-center">
        ...
        <div className="relative h-[128px] w-[128px] shrink-0">
          <svg
            ...
            aria-hidden="true"
          >
            <circle cx="60" cy="60" r="52" stroke="#efedf4" strokeWidth="12" fill="none" />
            <circle
              cx="60" cy="60" r="52" stroke="#6d28d9" strokeWidth="12"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <p className="mt-1 text-[10px] text-[#8b8697]">Overall Progress</p>
          </div>
        </div>
        ...
        {/* Line chart */}
        <div className="min-w-0">
          <svg viewBox="0 0 400 140" fill="none" className="h-[130px] w-full" aria-hidden="true">
            <defs>
            ...
            {gridlines.map(({ label, y }) => (
              <g key={label}>
                <line x1={X0} y1={y} x2="384" y2={y} stroke="#efedf4" strokeWidth="1" />
                <text x="34" y={y} textAnchor="end" dominantBaseline="middle" fontSize="11" fill="#a8a2b4">{label}</text>
              </g>
            ))}
            <polygon points={areaPoints} fill="url(#reikiGrad)" />
            <polyline points={linePoints} stroke="#6d28d9" strokeWidth="2" fill="none" />
            {points.map(({ x, y }) => (
              <circle key={x} cx={x} cy={y} r="3.2" fill="#6d28d9" />
            ))}
            {weeks.map((week, i) => (
              <text key={week} x={toX(i)} y="130" textAnchor="middle" fontSize="11" fill="#a8a2b4">{week}</text>
            ))}
          </svg>
        </div>
      </div>
