...
export default function ProfileCard() {
  ...
  const { data: subs } = useApi<any[]>(() => api.getSubscriptions());
      ...
      <div className="px-5 pt-7 pb-5">
        ...
        <div className="relative mx-auto h-[128px] w-[128px]">
          <span className="block h-full w-full overflow-hidden rounded-full ring-[3px] ring-[#e9b85c] ring-offset-[3px] ring-offset-white">
            <Image
              src={user.avatar || "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=256&q=80"}
              alt={user.name || "User"}
              width={128}
              height={128}
          ...
          >
            <Camera className="h-[16px] w-[16px] text-white" strokeWidth={1.9} />
          </button>
        </div>
        ...
        <div className="mt-[10px] flex justify-center">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-[#f1e9fb] px-[13px] py-[5px] text-[12px] font-semibold text-[#6b3fa0]">
            <Crown className="h-[13px] w-[13px] text-[#e9a11e]" fill="#e9a11e" strokeWidth={1.5} />
        ...
        <div className="mt-5 flex flex-col gap-[15px]">
          <div className="flex items-center gap-3 text-[13px] text-[#2b0f47]">
            <Mail className="h-[16px] w-[16px] shrink-0 text-[#6b3fa0]" strokeWidth={1.8} />
            <span>{user.email || "—"}</span>
          </div>
          <div className="flex items-center gap-3 text-[13px] text-[#2b0f47]">
            <Phone className="h-[16px] w-[16px] shrink-0 text-[#6b3fa0]" strokeWidth={1.8} />
            <span>{user.phone || "—"}</span>
          </div>
          <div className="flex items-center gap-3 text-[13px] text-[#2b0f47]">
            <MapPin className="h-[16px] w-[16px] shrink-0 text-[#6b3fa0]" strokeWidth={1.8} />
