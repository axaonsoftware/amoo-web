...
export default function AccountInformation() {
  ...
  const tiles = [
    ...
    },
    {
      label: "Reward Points",
      value: user.points
        ? `${Number(user.points).toLocaleString("en-IN")} Points`
        : "—",
      Icon: Star,
      circle: "bg-[#ece5fb]",
      icon: "text-[#6b3fa0]",
      ...
      {loading ? (
        ...
        <div className="mt-4 grid grid-cols-1 gap-[14px] sm:grid-cols-2 lg:grid-cols-4">
          {tiles.map(({ label, value, Icon, circle, icon }) => (
            ...
            >
              <span
                className={`flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-full ${circle}`}
              >
                <Icon
                  className={`h-[19px] w-[19px] ${icon}`}
                  strokeWidth={1.8}
                />
              </span>
              <div className="min-w-0 leading-[1.3]">
                <p className="whitespace-nowrap text-[12px] text-[#6c6b78]">
                  {label}
                </p>
                <p className="whitespace-nowrap text-[14px] font-semibold text-[#2b0f47]">
                  {value}
                </p>
              </div>
            </div>
      ...
      <div className="mt-[18px] flex items-center gap-6">
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between">
            <span className="text-[14px] font-semibold text-[#2b0f47]">
              Profile Completion
            </span>
          ...
          <p className="mt-[11px] text-[12.5px] text-[#6c6b78]">
            Complete your profile to get more accurate predictions and better
            recommendations.
          </p>
        </div>
