...
export default function StatsRow() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: formatCurrency(wallet.data?.balance ?? 0, wallet.data?.currency),
      action: "Add Money",
      tinted: false,
    },
    ...
    return (
      <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="animate-pulse rounded-[14px] border border-[#efe6d6] bg-white p-4 shadow-[0_2px_10px_rgba(42,17,72,.05)]"
          >
      ...
      {stats.map(({ Icon, label, value, action, tinted }) => (
        <div
          ...
          className={`flex items-start gap-3.5 rounded-[14px] border px-4 py-4 shadow-[0_2px_10px_rgba(42,17,72,.05)] ${
            tinted
              ? "border-[#e6dcf5] bg-[#f4edfc]"
              : "border-[#efe6d6] bg-white"
          }`}
        >
          <span className="flex h-[46px] w-[46px] shrink-0 items-center justify-center rounded-full bg-gradient-to-b from-[#5a2496] to-[#3d1268] shadow-[0_4px_10px_rgba(61,18,104,.3)]">
            <Icon
              className="h-[22px] w-[22px] text-[#f0c877]"
              strokeWidth={1.7}
            />
          </span>
          <div className="min-w-0">
            ...
            <Link
              href="/user-dashboard"
              className="mt-2.5 inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#6b3fa0]"
            >
              {action}
