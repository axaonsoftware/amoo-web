...
export default function RecentKundalis() {
  ...
  const rows: Report[] = (data?.data ?? []).filter(
  ...
  return (
    <section className="rounded-[14px] border border-[#f0e7d8] bg-white px-5 py-[18px] shadow-[0_1px_3px_rgba(43,15,71,.04)]">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-[19px] font-bold text-[#2b0f47]">Recent Kundalis</h2>
        <button
          type="button"
      ...
      {loading ? (
        ...
        <p className="mt-2 text-[12.5px] text-[#8b8697]">No kundali reports found.</p>
      ) : (
      <ul className="mt-2">
        {rows.map((r, i) => {
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-[3px] truncate text-[11.5px] text-[#6c6b78]">{r.type}</p>
              <p className="mt-[2px] truncate text-[11.5px] text-[#8b8697]">{dateStr}</p>
            </div>
            ...
            >
              <MoreVertical className="h-[16px] w-[16px]" strokeWidth={1.8} />
            </button>
          </li>
          );
        })}
