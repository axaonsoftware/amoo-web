...
if (typeof window !== "undefined" && process.env.NODE_ENV === "production" && !RAW_WHATSAPP) {
  console.warn("CRITICAL: NEXT_PUBLIC_WHATSAPP_NUMBER is not set. Using placeholder number.");
}
...
if (typeof window !== "undefined" && process.env.NODE_ENV === "production" && WHATSAPP_RAW.length < 10) {
  console.warn("CRITICAL: WhatsApp number appears invalid (digits:", WHATSAPP_RAW.length, ")");
}
...
export const SOCIAL_LINKS = {
  facebook: process.env.NEXT_PUBLIC_FACEBOOK_URL || "https://facebook.com/amoooguru",
  instagram: process.env.NEXT_PUBLIC_INSTAGRAM_URL || "https://instagram.com/amooguru_official",
  youtube: process.env.NEXT_PUBLIC_YOUTUBE_URL || "https://youtube.com/@amoooguru",
} as const;

export const BUSINESS_HOURS = process.env.NEXT_PUBLIC_BUSINESS_HOURS || "Mon - Sat: 10 AM - 8 PM";
